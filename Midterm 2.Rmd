...
# Normalize the aggregated data by the sum of all land use types within each province to get fractions
fraction_agg[, -1] <- fraction_agg[, -1] / rowSums(fraction_agg[, -1], na.rm = TRUE)
...
fraction_agg_sum <- rowSums(fraction_agg[ , 2:4], na.rm = TRUE)

fraction_agg_sum <- as.data.frame(fraction_agg_sum)

fraction_agg_sum

ggplot() +
  geom_histogram(data = fraction_agg_sum, aes(x = fraction_agg_sum))
