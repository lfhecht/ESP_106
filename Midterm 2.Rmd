...
fraction2 <- raster::extract(landuse_cropped, provinces, df = TRUE, fun = sum) 


fraction_agg <- aggregate(fraction2[, 3:ncol(fraction)], by = list(province_ID = fraction2$ID), FUN = sum)
...
for (i in 3:ncol(fraction_agg)) {
  fraction_agg[, i] <- fraction_agg[, i] / sum(fraction_agg[, i], na.rm = TRUE)
}

# Sum up the normalized fractions in the four land classes for each province
fraction_agg$Total <- rowSums(fraction_agg[, -1], na.rm = TRUE)
...
hist(fraction_agg$Total, main = "Total Land Use Fractions by Province",
     xlab = "Total Land Fraction", ylab = "Frequency", col = "skyblue")
...
fraction_agg <- aggregate(fraction[, 3:ncol(fraction)], by = list(province_ID = fraction$ID), FUN = sum)

# Step 3: Normalize the fractions by dividing each column by the sum of its values
for (i in 2:ncol(fraction_agg)) {
  fraction_agg[, i] <- fraction_agg[, i] / sum(fraction_agg[, i], na.rm = TRUE)
}

# Step 4: Calculate total fractions by summing up the normalized fractions for each province
fraction_agg$Total <- rowSums(fraction_agg[, -1], na.rm = TRUE)

# Plot the histogram of the total land use fractions
hist(fraction_agg$Total, main = "Total Land Use Fractions by Province",
     xlab = "Total Land Fraction", ylab = "Frequency", col = "skyblue")
```
...
# Extract land use values for each province
extracted_values <- raster::extract(landuse_cropped, provinces, df = TRUE)
